// Dron Capital - All-in-one server (frontend + Angel One proxy)
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (action === 'yahoo') {
      ...
      const data = await fetchJSON(`https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(symbols)}`);
...
server.listen(PORT, () => console.log(`Dron Capital running on port ${PORT}`));
